...
function buildCommand(options) {
  ...
  if (options.noExternals) command += ' --no-externals';
...
function analyze(options, callback) {
  ...
  exec(command + ' --reporter=json > ' + options.outputFileName, function(
    error,
    stdout,
    stderr
  ) {
    if (error !== null) {
      ...
      fs.readFile(options.outputFileName, function(error, data) {
        ...
        if (Object.keys(options['performanceBudget']).length > 0) {
          ...
          while (m_length--) {
            console.log(
              clc.black.bgYellowBright.underline(m[m_length]) +
                ' ' +
                clc.yellow.bgBlack(metrics[m[m_length]])
            );
          }
          callback();
        }
